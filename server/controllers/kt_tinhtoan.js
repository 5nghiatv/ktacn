...
exports.BaocaoBctcXML = function (req, res) {
  ...
  fs.readFile('public/download/' + req.query.filename, 'utf-8', (err, data) => {
    ...
    xml2js.parseString(data, (err, result) => {
      ...
      axios
        .post(process.env.APP_URL + '/api/ketquakd', req.query)
        .then((response) => {
          ...
          //========= TÍNH PL_LCTTGT
          axios
            .post(process.env.APP_URL + '/api/lctiente', req.query)
            .then((response) => {
              ...
              var cdtk =
                result.HSoThueDTu.HSoKhaiThue[0].PLuc[0].PL_LCTTGT[0].NamNay //PL_LCTTGT //PL_KQHDSXKD
              var cdtk_ =
                result.HSoThueDTu.HSoKhaiThue[0].PLuc[0].PL_LCTTGT[0].NamTruoc //PL_LCTTGT //PL_KQHDSXKD
              ...
              //========= TÍNH PL_LCTTGT
              axios
                .post(process.env.APP_URL + '/api/cdketoan', req.query)
                .then((response) => {
                  ...
                  runQuery(
                    ...
                    function (cdps) {
                      ...
                      var xml_cdkt =
                        result.HSoThueDTu.HSoKhaiThue[0].PLuc[0].PL_CDTK
                      if (req.query.company.dnlon == false) {
                        var cdtk =
                          result.HSoThueDTu.HSoKhaiThue[0].PLuc[0].PL_CDTK[0]
                            .SoDuDauKy[0].No
                        var znodk = 0,
                          zcodk = 0,
                          zpsno = 0,
                        ...
                        Object.keys(cdtk[0]).forEach((ctma, index) => {
                          if (ctma.substr(0, 2) == 'ct') {
                            ...
                            if (ctma == 'tongCong') {
                              ...
                              xml_cdkt[0].SoPhatSinhTrongKy[0].No[0][ctma] =
                                zpsno
                              xml_cdkt[0].SoPhatSinhTrongKy[0].Co[0][ctma] =
                                zpsco
              ...
              .then((response) => {
                ...
                axios
                  ...
                  .then((response) => {
                    ...
                    var colTM = req.query.company.dnlon
                      ? ['BF', 'BG', 'BH']
                      : ['L', 'M', 'N']
                    var colKQ = req.query.company.dnlon
                      ? ['AG', 'AV']
                      : ['H', 'J']
                      ...
                      if (
                        ...
                        if (row.getCell(colTM[0]).value == 'kqkd') {
                          ws_kqkd.eachRow(function (rowkq, rowNumberkq) {
                            if (
                              row.getCell(colTM[1]).value ===
                              rowkq.getCell('B').value
                            ) {
                              if (rowkq.getCell('D').value != 0)
                                row.getCell(colKQ[0]).value =
                                  rowkq.getCell('D').value
                              if (rowkq.getCell('E').value != 0)
                                row.getCell(colKQ[1]).value =
                                  rowkq.getCell('E').value
                            }
                          })
                        } else {
                          var colL = row
...
exports.dmsodutks = function (req, res) {
  runQuery(
    ...
    function (cdps) {
      if (
        typeof cdps != 'undefined' &&
        typeof cdps[0] != 'undefined' &&
        cdps[0].length != 0
      ) {
        // res.send(rows);
        return res.status(200).json({
              ...
              if (
                ...
                for (var k = 0; k < cdketoan.length; k++) {
                  if (
                    ...
                    if (
                      (cdketoan[k].masn != '421a') &
                      (cdketoan[k].masn != '421b')
                    ) {
                    ...
                    function (datant) {
                      ...
                      for (var k = 0; k < lctiente.length; k++) {
                        if (lctiente[k].maso == '50') {
                          global.cookie.kytruoc =
                            global.cookie.kytruoc - lctiente[k].kytruoc
                          global.cookie.kynay =
                            global.cookie.kynay - lctiente[k].kynay
                        }
                      }
        ...
        if (req.query.patern.includes('+')) {
          ...
          if (typeof datann != 'undefined') {
            datann[0].forEach((kq, index) => {
              ...
              ws.getCell('C' + sott).value = moment(kq.ngay).format(
                'DD/MM/YYYY',
              )
          ...
          runQuery(
            ...
            function (createtab) {
              ...
              var result = new Promise((resolve, reject) => {
                tables.forEach(async (table, index, array) => {
                  ...
                  for (var i = 0; i < createtab.length; i++) {
                    if (
                      createtab[i].length > 0 &&
                      createtab[i][0].Table == table
                    ) {
                  ...
                  cReturn +=
                    PHP_EOL + `DROP TABLE IF EXISTS ${table};` + PHP_EOL
              ...
              result.then((sql) => {
                ...
                if (params == 'undefined') {
                  var filename =
                    'Table_' + connect.database + '_' + connect.user + '.sql'
                } else {
