...
exports.getAllCustomer = function ( req, res){
  Customer.find((err, docs) => {
    if (!err) {
      return res.status(200).json({  success: true, message: 'A list of all Customer', customers: docs, });
  ...
  exports.createCustomer =  function (req, res) {
    ...
    cust.save(function(err){
        if(err){
          ...
          res.status(201).json({ success: true, message: 'New customer created successfully', customer: cust, });
        }
    });
