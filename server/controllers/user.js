...
const { createUserSql, getAllUserSql, getSingleUserSql,updateUserSql, deleteUserSql, findOneUserSql } = require('./kt_users');
  ...
  exports.getUserEmail = function (req, res) {
    ...
    if(!__mongodb){
      ...
      User.findOne( { email: req.params.email } )
        .then((singleUser) => {
          if(!singleUser) return res.status(404).json({ success: false, message: 'This User does not exist.',error: 404 });
          res.status(200).json({
            success: true,
