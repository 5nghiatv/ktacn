...
router.get('/backupdb',async (req, res) => {
    ...
    for (let index = 0; index < listColls.length; index++) {
        ...
        if(!itemname.includes(".")){
            ...
            .on("finish", function() { console.log(index+1,itemname+".csv");  })
            .pipe(ws);        
        }
      ...
      .on("end", async function() {
          ...
          //console.log(csvData);
            ...
            if (resCollct){
                ...
                client.close();
                res.status(200).json({success: "Collection ["+ clname +"] RESTORE Succesfully !!"  });
            }else{
    ...
    for (let index = 0; index < listColls.length; index++) {
        if(listColls[index].name == collect ) {
            ...
            myDB.collection(collect).drop(function(err, delOK) {
                client.close();
                if (!err){
                    return res.status(200).json({success: "Collection ["+ collect +"] DELETE Succesfully !!"  });
                }else{
                    return res.status(200).json({ success: "Collection ["+ collect +"] Delete ERROR !!" });
                }
            });
        }
        ...
        if(listColls.length == index +1 && !collexist) {
            client.close();
            return res.status(200).json({ success: "Collection ["+ collect +"] Not exist. Please RESTORE... " });
        }
    }
});
